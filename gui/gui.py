...
class App(ctk.CTk):
    def __init__(self, title, size):
        ...
        # widgets
        self.sidebar = SideBar(self, width=180)
...
class SideBar(ctk.CTkFrame):
    ...
    def create_widgets(self, parent):
        ...
        ctk.CTkButton(self, text="Categories", width=150).pack(pady=5)
        ctk.CTkButton(self, text="Products", width=150, command=None).pack(pady=5)
...
class Main(ctk.CTkFrame):
    def __init__(self, parent, **kwargs):
        ...
        # self.category_tab = ctk.CTkFrame(self)
...
class ProductTab(ctk.CTkFrame):
        def __init__(self, parent):
            ...
            # Header/Label
            ctk.CTkLabel(self, text="Product Management", font=("Arial", 20, "bold")).pack(pady=10)

            self.product_table = ttk.Treeview(
                self, columns=("name", "price", "category"), show="headings")
            self.product_table.heading("name", text="Product Name")
            self.product_table.heading("price", text="Price (₱)")
            self.product_table.heading("category", text="Category")
            self.product_table.pack(expand=True, fill="both", padx=10, pady=(10, 5))
